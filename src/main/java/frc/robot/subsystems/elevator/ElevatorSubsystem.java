...
public class ElevatorSubsystem extends SubsystemBase {
    ...
    public void goToSetpoint(double setpoint) {
        ...
        if (RobotBase.isReal()) {
            elevatorMotor1Controller.setReference(limitedSetpoint, ControlType.kMAXMotionPositionControl,
    ...
    public Command huCommand(int level) {
        return new InstantCommand(() -> {
            double setpoint;
            if (RobotBase.isReal()) {
                if (level == 1) {
                    setpoint = ElevatorConstants.HeightSetpoints.Algae.L1;
                } else if (level == 3) {
                    setpoint = ElevatorConstants.HeightSetpoints.Algae.L3;
                } else {
                    setpoint = ElevatorConstants.HeightSetpoints.HOME;
    ...
    public void periodic() {
        if (RobotBase.isReal()) {
            ...
            SmartDashboard.putBoolean("Elevator At Max Height",
                    elevatorMotor1.getEncoder().getPosition() <= ElevatorConstants.ELEVATOR_MAX_HEIGHT);
            SmartDashboard.putBoolean("Elevator At Min Height",
                    elevatorMotor1.getEncoder().getPosition() >= ElevatorConstants.ELEVATOR_MIN_HEIGHT);
            ...
            if (Math.abs(currentPos - targetSetpoint) > 0.5) {
                elevatorMotor1Controller.setReference(targetSetpoint, ControlType.kMAXMotionPositionControl,
                        ClosedLoopSlot.kSlot0, 0);
            }

            // Eğer encoder değeri limitlerin ötesindeyse, motoru durdur
            if (currentPos < ElevatorConstants.ELEVATOR_MAX_HEIGHT
                    || currentPos > ElevatorConstants.ELEVATOR_MIN_HEIGHT) {
                elevatorMotor1.set(0);
            }
        }
    }
