...
public class RobotContainer {
        ...
        private void configureButtonBindings() {
                ...
                new POVButton(operatorJoystick, 0) // L3
                                .onTrue(AutomatedScoring.coralShootCommand(3, wristSubsystem, elevatorSubsystem,
                                                shooterSubsystem));
                new POVButton(operatorJoystick, 90) // L2
                                .onTrue(AutomatedScoring.coralShootCommand(2, wristSubsystem, elevatorSubsystem,
                                                shooterSubsystem));
                new POVButton(operatorJoystick, 180) // L1
                                .onTrue(AutomatedScoring.coralShootCommand(1, wristSubsystem, elevatorSubsystem,
                                                shooterSubsystem));
                new JoystickButton(operatorJoystick, 14) //L0
                                .onTrue(AutomatedScoring.coralShootCommand(0, wristSubsystem, elevatorSubsystem,
                                                shooterSubsystem));

                new POVButton(operatorJoystick, 270)// intake
                                .onTrue(AutomatedScoring.intakeCommand(0, wristSubsystem, elevatorSubsystem,
                                                shooterSubsystem));

                new JoystickButton(operatorJoystick, 5)
                                .onTrue(AutomatedScoring.algeaPickUpCommand(2, 0.5, wristSubsystem,
                                                elevatorSubsystem, shooterSubsystem));

                new JoystickButton(operatorJoystick, 7)
                                .onTrue(AutomatedScoring.algeaPickUpCommand(3, 0.5, wristSubsystem,
                                                elevatorSubsystem, shooterSubsystem));

                new JoystickButton(operatorJoystick, 6)
                                .onTrue(AutomatedScoring.scoreWristAlgeaPathing(1, wristSubsystem));
                new JoystickButton(operatorJoystick, 2)
                                .onTrue(AutomatedScoring.stopShooterCommand( shooterSubsystem));

                new JoystickButton(operatorJoystick, 3)
                                .onTrue(AutomatedScoring.shootl1Command(0.2, shooterSubsystem));

                new JoystickButton(operatorJoystick, 8)
                                .onTrue(AutomatedScoring.scoreElevatorAlgea(5, elevatorSubsystem));
